// 由 scripts/build_data.py 從正式 Excel 名冊產生，請勿手動編輯。
window.STAFF_DATA = [
  {
    "name": "洪金英",
    "title": "校長",
    "group": "指揮部",
    "fireGroup": "-",
    "role": "指揮官",
    "detail": "負責指揮、督導、協調。\n依情況調動各分組間相互支援。",
    "source": "應變小組主表"
  },
  {
    "name": "陳佑昌",
    "title": "教務主任",
    "group": "指揮部",
    "fireGroup": "-",
    "role": "指揮官代理人",
    "detail": "於校長不在學校或因故無法執行指揮官職務時，擔任指揮官之任務。",
    "source": "應變小組主表"
  },
  {
    "name": "簡世煜",
    "title": "學務主任",
    "group": "指揮部",
    "fireGroup": "-",
    "role": "發言人",
    "detail": "負責統一對外發言。\n呈報上級主管相關通報事宜。\n襄助指揮官指揮、督導及協調等事宜。",
    "source": "應變小組主表"
  },
  {
    "name": "陳政川",
    "title": "總務主任",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組長",
    "detail": "平時急救常識宣導。\n檢修與保養救災相關裝備。\n受災教職員工生之搶救及搜救。\n清除障礙物協助逃生。\n協助疏散未能及時避難教職員工生。\n關閉校區總電源及瓦斯。\n設置警示標誌及交通管制。\n毀損建築物與設施之警示標誌。\n支援避難引導組及搬運防災救急箱器材。\n如發生火災，研判火勢，必要時使用滅火器、室內消防栓進行初期滅火工作。",
    "source": "應變小組主表"
  },
  {
    "name": "林承恩",
    "title": "設備組長",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "徐嘉靖",
    "title": "衛生組長",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "梁弘又",
    "title": "學務創新人力",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "黃怡惠",
    "title": "學務創新人力",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "林維儀",
    "title": "學務創新人力",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "劉泓均",
    "title": "學務創新人力",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "連志航",
    "title": "體育組長",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "李子蘊",
    "title": "圖書幹事",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "鄭世甲",
    "title": "技工",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "林愛玲",
    "title": "工友",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "謝美齡",
    "title": "工友",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "林恩昕",
    "title": "教務管理員",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "吳凱煜",
    "title": "教務技佐",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "協助搶救組各項救災與搜救工作",
    "source": "應變小組主表"
  },
  {
    "name": "吳家瑋",
    "title": "研發處主任",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組長",
    "detail": "通報地方救災、治安、醫療及聯絡有關人員等，並請求支援。\n通報教育行政主管機關（教育局處）、縣市政府災害應變中心、鄉/鎮/市/區災害應變中心及教育部校園安全暨災害防救通報處理中心，已疏散人數、收容地點、災情等。\n負責蒐集、評估、傳播和使用有關於災害、資源與狀況發展的資訊。\n蒐集並紀錄指揮官所有下達應變指令。\n回報災情狀況。\n啟動社區志工與家長協助。\n學生家長必要緊急聯繫。",
    "source": "應變小組主表"
  },
  {
    "name": "楊麗雯",
    "title": "會計主任",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "廖麗雅",
    "title": "實研組長",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "張淑茹",
    "title": "訓育組長",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "李欣桂",
    "title": "資訊媒體組長",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "傅永吉",
    "title": "總務約僱書記",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "邱卉綺",
    "title": "註冊組長",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "鄭樂",
    "title": "課務組長",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "林恩昕",
    "title": "教務管理員",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "吳相瑩",
    "title": "總務約僱幹事",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "陳藍怡",
    "title": "教務約僱幹事",
    "group": "通報組",
    "fireGroup": "通報班",
    "role": "組員",
    "detail": "協助通報及災害資訊蒐集與回報",
    "source": "應變小組主表"
  },
  {
    "name": "王聖淵",
    "title": "圖書館主任",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組長",
    "detail": "依據不同災害之應變原則，協助教職員工生進行第一時間的避難。\n於適當時機，協助教職員工生緊急疏散至集結點。\n避難人數清點確認。\n維護教職員工生及集結點安全。\n進行必要的安撫。\n視災情變化，引導教職員工生移動、避難與安置。\n隨時清查教職員工生人數與安全狀況，並回報或申請救護車支援。\n在集結地點設置服務臺，提供協助與諮詢。\n學生領回作業。",
    "source": "應變小組主表"
  },
  {
    "name": "周峻民",
    "title": "高中生輔組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "柯明廷",
    "title": "國中生教組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "曾淑娟",
    "title": "文書組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "9月1日到職",
    "source": "應變小組主表"
  },
  {
    "name": "徐珮華",
    "title": "出納組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "黃郁芬",
    "title": "學務創新人力",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "丁宇沛",
    "title": "人事助理員",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "翁敏召",
    "title": "事務組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "張家瑋",
    "title": "圖書約僱幹事",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "陳佳琪",
    "title": "教學組長",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "蔡宜穎",
    "title": "學務約僱管理員",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "王建民",
    "title": "研發處前導助理",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "協助引導教職員工生避難疏散與秩序維護",
    "source": "應變小組主表"
  },
  {
    "name": "王秋堯",
    "title": "人事主任",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組長",
    "detail": "建築物及設施安全檢查。\n教職員工生需要臨時收容時，協助發放生活物資、糧食及飲用水；以及各項救災物資登記、造冊、保管及分配。\n協助設置警示標誌及交通管制。\n協助毀損建築物與設施之警示標誌。\n校區硬體復舊及安全維護。\n維護臨時收容空間安全。\n確認停班、停課後，確實疏散校園內人員。\n防救災設施操作。",
    "source": "應變小組主表"
  },
  {
    "name": "王為倩",
    "title": "會計組員",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "鄭世甲",
    "title": "技工",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "黃月英",
    "title": "會計約僱佐理員",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "黃怡惠",
    "title": "學務創新人力",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "林愛玲",
    "title": "工友",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "李桂鳳",
    "title": "教務幹事",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "蔣荌云",
    "title": "總務專案行政助理",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "莊益翔",
    "title": "總務書記",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "許杏如",
    "title": "學務幹事",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "李研懷",
    "title": "生物代理教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "張家瑋",
    "title": "圖書約僱幹事",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "李子蘊",
    "title": "圖書幹事",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "協助校園安全防護與物資發放作業",
    "source": "應變小組主表"
  },
  {
    "name": "林巧芳",
    "title": "輔導主任",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組長",
    "detail": "設立急救站。\n針對傷患進行檢傷分類。\n緊急基本急救、重傷患就醫護送。\n情緒支持、安撫及心理輔導。\n登記傷患姓名、班級，建立傷患名冊。",
    "source": "應變小組主表"
  },
  {
    "name": "林淳雅",
    "title": "護理師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與檢傷分類作業",
    "source": "應變小組主表"
  },
  {
    "name": "陳玫嫣",
    "title": "輔導幹事",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與心理輔導",
    "source": "應變小組主表"
  },
  {
    "name": "謝嘉軒",
    "title": "輔導組長",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與心理輔導",
    "source": "應變小組主表"
  },
  {
    "name": "蕭如宜",
    "title": "資料組長",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與資料登記",
    "source": "應變小組主表"
  },
  {
    "name": "曾聖峰",
    "title": "特教組長",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與特教生照護",
    "source": "應變小組主表"
  },
  {
    "name": "邱卉綺",
    "title": "註冊組長",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與資料登記",
    "source": "應變小組主表"
  },
  {
    "name": "邱欣怡",
    "title": "教務幹事",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與行政支援",
    "source": "應變小組主表"
  },
  {
    "name": "趙梨吟",
    "title": "護理師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助傷患救護與檢傷分類作業",
    "source": "應變小組主表"
  },
  {
    "name": "呂政憲",
    "title": "高中輔導教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助心理輔導與情緒支持",
    "source": "應變小組主表"
  },
  {
    "name": "宋欣祐",
    "title": "高中輔導教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助心理輔導與情緒支持",
    "source": "應變小組主表"
  },
  {
    "name": "任以容",
    "title": "國中輔導教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助心理輔導與情緒支持",
    "source": "應變小組主表"
  },
  {
    "name": "白驊中",
    "title": "特教教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助特教學生緊急照護",
    "source": "應變小組主表"
  },
  {
    "name": "柳嘉玲",
    "title": "特教教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助特教學生緊急照護",
    "source": "應變小組主表"
  },
  {
    "name": "廖芷瑩",
    "title": "高中特教導師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助特教學生緊急照護",
    "source": "應變小組主表"
  },
  {
    "name": "張又心",
    "title": "國中專輔代理教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助心理輔導與情緒支持",
    "source": "應變小組主表"
  },
  {
    "name": "蕭雅心",
    "title": "國中專輔代理教師",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組員",
    "detail": "協助心理輔導與情緒支持",
    "source": "應變小組主表"
  },
  {
    "name": "蘇蕙冠",
    "title": "國文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "平時急救常識宣導。\n檢修與保養救災相關裝備。\n受災教職員工生之搶救及搜救。\n清除障礙物協助逃生。\n協助疏散未能及時避難教職員工生。\n關閉校區總電源及瓦斯。\n設置警示標誌及交通管制。\n毀損建築物與設施之警示標誌。\n支援避難引導組及搬運防災救急箱器材。\n如發生火災，研判火勢，必要時使用滅火器、室內消防栓進行初期滅火工作。\n註：如果教師於任課期間發生災害，請隨班進行疏散。",
    "source": "附表一_高中專任"
  },
  {
    "name": "李瑋菁",
    "title": "國文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "李蕙蘭",
    "title": "英文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "陳儀君",
    "title": "數學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "陳柏均",
    "title": "物理教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "蘇順聖",
    "title": "數學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "游正墶",
    "title": "數學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "張智詠",
    "title": "物理教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "葉承憲",
    "title": "物理教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "李曜宇",
    "title": "化學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "簡嘉慧",
    "title": "生物教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "黃致柔",
    "title": "地科教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "杜映臻",
    "title": "歷史教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "吳亭葦",
    "title": "公民(社會)教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "林若蘋",
    "title": "音樂教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "朱秀勇",
    "title": "資訊概論教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "李怡璇",
    "title": "資訊概論教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "連時宜",
    "title": "美術教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "呂政憲",
    "title": "輔導教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "黃怡衡",
    "title": "健護教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "蘇婉如",
    "title": "國文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "楊森吉",
    "title": "數學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "陳素梅",
    "title": "體育教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "許瀚元",
    "title": "化學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "陳瑩璇",
    "title": "化學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "連紫汝",
    "title": "數學教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "謝文瑜",
    "title": "英文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "劉俐君",
    "title": "歷史教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "呂惠甄",
    "title": "國文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "黃丞佑",
    "title": "體育教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "吳柏萱",
    "title": "數學214導師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "連文伶",
    "title": "英文進修教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "林愷晙",
    "title": "國文進修教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "宋欣祐",
    "title": "輔導教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "謝渟榛",
    "title": "英文教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "陳柏廷",
    "title": "地理育嬰教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "林儀靜",
    "title": "體育教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "黃宥愷",
    "title": "生物教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "Ｍalach Bentley Gallegos",
    "title": "外籍教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "Oppel Le Clue",
    "title": "外籍教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "Martini Matthew Ryan",
    "title": "外籍教師",
    "group": "搶救組",
    "fireGroup": "滅火班",
    "role": "組員",
    "detail": "同搶救組負責工作（詳見首列說明）",
    "source": "附表一_高中專任"
  },
  {
    "name": "101班",
    "title": "吳林建宏",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "依據不同災害之應變原則，協助教職員工生進行第一時間的避難。\n於適當時機，協助教職員工生緊急疏散至集結點。\n避難人數清點確認。\n維護教職員工生及集結點安全。\n進行必要的安撫。\n視災情變化，引導教職員工生移動、避難與安置。\n隨時清查教職員工生人數與安全狀況，並回報或申請救護車支援。\n在集結地點設置服務臺，提供協助與諮詢。\n學生領回作業。\n註：如果教師任課期間發生災害，請隨班進行疏散。\n註：如果教師當節無課務，請導師們隨避難引導組集合點名，並進行防災工作。",
    "source": "附表二_國高中導師"
  },
  {
    "name": "102班",
    "title": "張韻翊",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "103班",
    "title": "陳佩如",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "104班",
    "title": "謝采芸",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "105班",
    "title": "凃世元",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "106班",
    "title": "張瓊文",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "107班",
    "title": "徐慈華",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "108班",
    "title": "吳芊穎",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "109班",
    "title": "洪嘉壑",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "110班",
    "title": "周允心",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "111班",
    "title": "黃妙慈",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "112班",
    "title": "吳民惠",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "113班",
    "title": "朱芳德",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "201班",
    "title": "鄭偲妙",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "202班",
    "title": "林易昇",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "203班",
    "title": "胡傑明",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "204班",
    "title": "程妤琪",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "205班",
    "title": "林姿君",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "206班",
    "title": "楊莉莉",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "207班",
    "title": "蕭柔茵",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "208班",
    "title": "林奕安",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "209班",
    "title": "陳琪",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "210班",
    "title": "陳巧雯",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "211班",
    "title": "方思閔",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "212班",
    "title": "林建融",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "213班",
    "title": "李彩滿",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "301班",
    "title": "王嘉瑜",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "302班",
    "title": "蔡育螢",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "303班",
    "title": "游博鈞",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "304班",
    "title": "康家寧",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "305班",
    "title": "林婉平",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "306班",
    "title": "游硯舒",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "307班",
    "title": "羅瑞珍",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "308班",
    "title": "郭雅婷",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "309班",
    "title": "周宜賢",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "310班",
    "title": "黃品菁",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "311班",
    "title": "吳秋慧",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "312班",
    "title": "吳易哲",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "313班",
    "title": "李台英",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "701班",
    "title": "許綉玉",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "702班",
    "title": "湯易芝",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "703班",
    "title": "范美如",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "704班",
    "title": "杜孟純",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "705班",
    "title": "陳怡雯",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "706班",
    "title": "涂麗淑",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "801班",
    "title": "林昕萮",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "802班",
    "title": "曹文秋",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "803班",
    "title": "徐羚玉",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "804班",
    "title": "郭合益",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "805班",
    "title": "陳羿芬",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "806班",
    "title": "郭叡昌",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "901班",
    "title": "林婷婷",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "902班",
    "title": "邵詩芸",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "903班",
    "title": "許修瑜",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "904班",
    "title": "林尚穎",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "905班",
    "title": "廖惠如",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "906班",
    "title": "蔡宜恩",
    "group": "避難引導組",
    "fireGroup": "避難引導班",
    "role": "組員",
    "detail": "同避難引導組負責工作（詳見首列說明）",
    "source": "附表二_國高中導師"
  },
  {
    "name": "白驊中",
    "title": "特教教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "建築物及設施安全檢查。\n教職員工生需要臨時收容時，協助發放生活物資、糧食及飲用水；以及各項救災物資登記、造冊、保管及分配。\n協助設置警示標誌及交通管制。\n協助毀損建築物與設施之警示標誌。\n校區硬體復舊及安全維護。\n維護臨時收容空間安全。\n確認停班、停課後，確實疏散校園內人員。\n防救災設施操作。\n註：如果教師於任課期間發生災害，請隨班進行疏散。",
    "source": "附表三_國中專任"
  },
  {
    "name": "陳簾筑",
    "title": "國文教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "詹君琪",
    "title": "英文教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "許玉珍",
    "title": "數學教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "許倚瑄",
    "title": "數學教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "廖家立",
    "title": "理化教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "劉綵嵐",
    "title": "家政教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "黃稚雯",
    "title": "特教教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "葉麗芳",
    "title": "健康教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "巫麗真",
    "title": "音樂教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "楊宗翰",
    "title": "生科教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "馮瑞琦",
    "title": "國文教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "周美雲",
    "title": "理化教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "張呂岳",
    "title": "體育教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "柳嘉玲",
    "title": "特教教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "林平平",
    "title": "英文教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "秦旭琨",
    "title": "國文教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "任以容",
    "title": "輔導教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "許王穎",
    "title": "公民教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "柳怡雯",
    "title": "資訊教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "邱春英",
    "title": "社會教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "張又心",
    "title": "專輔教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "蕭雅心",
    "title": "專輔教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "李研懷",
    "title": "生物教師",
    "group": "安全防護組",
    "fireGroup": "安全防護班",
    "role": "組員",
    "detail": "同安全防護組負責工作（詳見首列說明）",
    "source": "附表三_國中專任"
  },
  {
    "name": "張安莛",
    "title": "秘書",
    "group": "緊急救護組",
    "fireGroup": "救護班",
    "role": "組長",
    "detail": "尚待補充",
    "source": "補充資料"
  }
];
